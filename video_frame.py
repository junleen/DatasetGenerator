...
##################################################
# Tools: to assistant image saving and processing
##################################################
...
def video2images(video_path,
    ...
    while ret:
        current_frame = os.path.join(save_path['img'], class_name, '%s_%.5d_img.png' % (class_name, frame_idx))
        ...
        if len(landmarks) != 0:
            ...
            # save
            save_image(join(save_path['img'], class_name), '%s_%.5d_img.png' % (video_name, frame_idx), face_resize)
            save_image(join(save_path['line'], class_name), '%s_%.5d_line.png' % (video_name, frame_idx), line)
            save_image(os.path.join(save_path['region'], class_name), '%s_%.5d_region.png' % (video_name, frame_idx), region)
            save_image(os.path.join(save_path['face'], class_name), '%s_%.5d_face.png' % (video_name, frame_idx), face)
...
def main(args):
    ...
    # 定义保存路径
    save_path = dict()
    save_path['root'] = args.output_dir; mkdir(save_path['root'])
    save_path['img'] = os.path.join(save_path['root'], 'img'); mkdir(save_path['img'])
    save_path['line'] = os.path.join(save_path['root'], 'line'); mkdir(save_path['line'])
    save_path['region'] = os.path.join(save_path['root'], 'region'); mkdir(save_path['region'])
    save_path['face'] = os.path.join(save_path['root'], 'face'); mkdir(save_path['face'])
    ...
    for (class_idx, video_path) in tqdm(videofolder.samples):
        ...
        frame_idx = video2images(video_path=video_path,
                                 save_path=save_path,
                                 class_name=classname,
                                 image_size=args.image_size,
                                 fixed_box=fixed_box,
                                 conditioned_box=conditioned_box,
                                 scale=args.scale)
        idx_logger[classname] += frame_idx

    # travel the classes, generate aus and files
    for class_name in tqdm(videofolder.classes):
        ia = join(args.output_dir, 'img', class_name)
        op = join(args.output_dir, 'aus', class_name)
    ...
    # print information
    print('*'*10, '\nProcessec images count:')
    for key in idx_logger:
        print('{}\t\t:{} images'.format(key, idx_logger[key]))
