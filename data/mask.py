...
def generate_line_mask(image, landmarks):
    ...
    # nose
    linemask = connect_line(linemask, landmarks[27:31])
...
def generate_region_mask(image, landmarks):
    regionmask = np.zeros(shape=image.shape[0:2]).astype(np.uint8)
    '''
        Use five landmarks
...
def generate_facial_mask(image, landmarks):
    ...
    :return: a mask map
    '''
    facemask = np.zeros(shape=image.shape[0:2]).astype(np.uint8)
    # ----- layer1: face region
...
def generate_eye_mask(image, landmarks):
    ...
    :return: a mask map
    '''
    eyemask = np.zeros(shape=image.shape[0:2]).astype(np.uint8)
    '''
        Use five landmarks
        w = (five_landmarks[0, 1] - five_landmarks[1, 1]) / (five_landmarks[0, 0] - five_landmarks[1, 0])
        b = five_landmarks[0, 1] - five_landmarks[0, 0] * w
...
def generate_mouth_mask(image, landmarks):
    ...
    '''
    # ----- layer3: mouth
    mouthmask = np.zeros(shape=image.shape[0:2]).astype(np.uint8)
